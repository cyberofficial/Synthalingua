...
$env:CL = "/Zm5000 /bigobj"; python -m nuitka --standalone --windows-console-mode=force --output-dir="dist\main_release" --enable-plugin=torch --enable-plugin=numpy --plugin-enable=multiprocessing --include-package=whisper --include-package=openvino --include-package=librosa --include-package=optimum --include-package=optimum.intel.openvino --include-distribution-metadata=click --include-distribution-metadata=datasets --include-distribution-metadata=flask --include-distribution-metadata=fsspec --include-distribution-metadata=humanize --include-distribution-metadata=networkx --include-distribution-metadata=nuitka --include-distribution-metadata=numba --include-distribution-metadata=numpy --include-distribution-metadata=onnxruntime --include-distribution-metadata=onnxscript --include-distribution-metadata=openvino --include-distribution-metadata=optimum --include-distribution-metadata=pandas --include-distribution-metadata=pip --include-distribution-metadata=prettytable --include-distribution-metadata=pycountry --include-distribution-metadata=pygments --include-distribution-metadata=pyinstaller --include-distribution-metadata=pyreadline3 --include-distribution-metadata=scipy --include-distribution-metadata=setuptools --include-distribution-metadata=sympy --include-distribution-metadata=torch --include-distribution-metadata=transformers --include-distribution-metadata=websockets --include-distribution-metadata=werkzeug --include-module=modules.transcribe_worker --include-data-file="modules/transcribe_worker.py=modules/transcribe_worker.py" --include-data-dir="html_data=html_data" --include-data-dir="data_whisper\Lib\site-packages\faster_whisper=faster_whisper" --include-data-dir="data_whisper\Lib\site-packages\optimum=optimum" --follow-imports --nofollow-import-to=yt_dlp.lazy_extractors --windows-icon-from-ico="e:\Synthalingua\Synthalingua_Wrapper\syntha.ico" --file-version="1.1.1.7" --product-version="1.1.1.7" --company-name="Cyber's Apps" --product-name="Synthalingua Beta 7" --file-description="Real-time Audio Transcription and Translation" --mingw64 --clang --show-progress synthalingua.py
