...
def parse_arguments():
    ...
    parser.add_argument("--record_timeout", default=1, help="Recording buffer timeout in seconds. Lower values (e.g., 1-2) provide more real-time processing but may cut off words. Higher values (e.g., 3-5) allow for more complete phrases but introduce processing delay. Recommended: 2-3 seconds for balanced performance.", type=float)
    parser.add_argument("--phrase_timeout", default=5, help="Silence duration in seconds before considering audio as a new phrase/sentence. When this timeout is reached, accumulated audio is processed and the buffer is cleared. Lower values (1-2s) process shorter phrases quickly, higher values (5-10s) wait for longer complete sentences. Recommended: 2-5 seconds depending on speech patterns.", type=float)
    parser.add_argument("--no_log", action='store_true', help="Minimize console output to show only the final transcription result. Hides intermediate processing messages, status updates, and verbose logging. Useful for clean output when piping results to other programs or when you only need the final text. Does not affect file output or error messages.")
    parser.add_argument("--debug", action='store_true', help="Enable detailed debugging output for troubleshooting. Shows internal processing steps, model loading details, audio chunk information, timing data, and error traces. Use when experiencing issues with transcription accuracy, performance problems, or unexpected behavior. Significantly increases console output volume.")
    parser.add_argument("--translate", action='store_true', help="Automatically translate transcribed text to English using built-in translation capabilities. Works with both live microphone input and file processing. Translation occurs after transcription, so you'll see both original and translated text. Quality depends on source language - works best with major world languages.")
    parser.add_argument("--transcribe", action='store_true', help="Force transcription mode instead of real-time processing. Useful for ensuring complete processing of audio files rather than streaming analysis. Automatically enabled for file inputs, but can be explicitly set for microphone input to use batch processing instead of continuous streaming.")
    parser.add_argument("--language", help="Source language of the audio content for improved transcription accuracy. Use language codes like 'en' (English), 'es' (Spanish), 'fr' (French), 'de' (German), 'ja' (Japanese), etc. When specified, Whisper skips language detection and uses this language directly, improving speed and accuracy for known content.", type=str, choices=VALID_LANGUAGES)
    ...
    parser.add_argument("--device", default="cuda", help="Processing device for AI model inference. 'auto' automatically selects best available device, 'cuda' uses NVIDIA GPU (fastest), 'cpu' uses processor (universally compatible), 'intel-igpu' uses Intel integrated graphics, 'intel-dgpu' uses Intel discrete GPU, 'intel-npu' uses Intel Neural Processing Unit. GPU acceleration significantly improves processing speed. Falls back to CPU if specified device unavailable.")
    parser.add_argument("--cuda_device", default=0, help="Specific NVIDIA GPU device ID when multiple GPUs are available. Use 0 for first GPU, 1 for second GPU, etc. Check available GPUs with 'nvidia-smi' command. Only relevant when --device is set to 'cuda'. Single GPU systems should use default value 0.", type=int)
    parser.add_argument("--discord_webhook", default=None, help="Discord webhook URL for sending live transcription results to a Discord channel. Format: https://discord.com/api/webhooks/[ID]/[TOKEN]. Useful for live streaming integration, remote monitoring, or collaborative transcription. Test webhook URL in Discord before using. Only works with real-time microphone mode.", type=str)
    parser.add_argument("--list_microphones", action='store_true', help="Display all available audio input devices (microphones) on your system and exit. Shows device names, IDs, and sample rates. Use this to identify the correct microphone name for --set_microphone or --microphone_enabled options. Helpful for troubleshooting audio input issues.")
    parser.add_argument("--set_microphone", default=None, help="Set the default microphone device by exact name (case-sensitive). Use --list_microphones first to see available device names. Example: 'Microphone (USB Audio Device)'. This becomes the primary audio input source. Useful when you have multiple microphones and want to specify which one to use.", type=str)
    parser.add_argument("--microphone_enabled", default=None, help="Enable a specific microphone by name and start real-time transcription immediately. Similar to --set_microphone but automatically begins listening. Use exact device name from --list_microphones output. Convenient for scripted or automated transcription setups.", type=str)
    parser.add_argument("--mic_chunk_size", default=1, help="Number of audio chunks to collect before processing a batch when using microphone input with --paddedaudio enabled. Example: --mic_chunk_size 2 --paddedaudio 1 will process 2 new chunks + 1 previous chunk (3 total) per batch. Higher values provide more context but increase processing delay. Set to 1 for immediate processing. Recommended: 1-3 chunks.", type=int)
    parser.add_argument("--model_dir", default="models", help="Directory path where AI models are stored and downloaded. Models can be several GB in size, so ensure adequate disk space. Use absolute path for custom locations (e.g., 'D:/AI_Models') or relative path from script location. Models are automatically downloaded on first use and cached for subsequent runs.")
    ...
    parser.add_argument("--intelligent_mode", action='store_true', help="If enabled, the system will automatically determine if the current output is below accuracy threshold and switch to a larger model for improved transcription quality.")
