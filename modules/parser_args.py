...
def parse_arguments():
    ...
    file_grp.add_argument("--timeout", type=int, default=0, help="Maximum time in seconds allowed for individual transcription jobs to complete before being terminated. Set to 0 (default) for no timeout - jobs run until completion. Useful for preventing stuck jobs in batch processing, setting time limits for large files, or ensuring predictable completion times. Works with both standard batch processing (--batchmode) and adaptive batch processing (--adaptive_batch). Example: --timeout 600 allows 10 minutes per job. Jobs exceeding this limit are killed and marked as failed. Only affects file processing modes, not real-time streaming or microphone input.")
    file_grp.add_argument("--fixsrt", type=str, help="Fix SRT subtitle file with incorrect timestamp ordering. Takes a path to an SRT file and creates a new file with '_repaired' suffix containing chronologically ordered subtitles. Useful for fixing SRT files where subtitles appear out of temporal sequence despite sequential numbering.")
